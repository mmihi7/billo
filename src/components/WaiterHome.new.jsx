...
import { getWaiters } from '@/lib/database';
import { 
  User, Loader2, Bug, Wifi, WifiOff, RefreshCw, 
  ArrowLeft, AlertCircle, Info, CheckCircle2 
} from 'lucide-react';
import { checkWaitersCollection, testFirestoreConnection } from '@/lib/database';
...
function WaiterHome() {
  ...
  const loadWaiters = useCallback(async () => {
    try {
      ...
      if (!restaurantId && currentUser?.uid) {
        const userRestaurant = await getRestaurantByOwner(currentUser.uid);
        if (userRestaurant) {
  ...
  const handleWaiterClick = (waiter) => {
    navigate(`/waiter/${waiter.id}`, { state: { waiter } });
  };
...
// Helper function to get restaurant by owner
